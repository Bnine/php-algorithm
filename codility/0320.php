...
function solution($X, $A) {
    ...
    foreach ($A as $key=>$val) {
        ...
        echo "frogPoint -> ".$frogPoint.PHP_EOL; 
        if ($frogPoint === $val) {
            echo "checking frog point".PHP_EOL; 
            if (!in_array($val, $currentArr)) {
                $currentArr[] = $val;
                $frogPoint++;
                echo "frogPoint updated! -> ".$frogPoint.PHP_EOL; 
            }
        }
    ...
    */
    ...
    foreach ($A as $key=>$val) {
        echo "key -> ".$key." & val -> ".$val.PHP_EOL; 
        if (!in_array($val, $currentArr)) {
            echo "insert array updated! val -> ".$val.PHP_EOL; 
            $currentArr[] = $val;
        }

        if (max($currentArr) === $X && count($currentArr) === $X) {
